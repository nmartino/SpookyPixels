...
func _would_cross_existing_path(i: int, j: int, room: Room) -> bool:
	...
	#if j ==0 no hay vecino izquierdo
	if j > 0:
		left_neighbour = map_data[i][j - 1]
	
	#if j == MAP_WIDTH -1, no hay vecino derecho
	if j < MAP_WIDTH - 1:
		right_neighbour = map_data[i][j + 1]
	
	#no pueden cruzarse direccion derecha si el vecino de la derecha va a la izquierda
	if right_neighbour and room.column > j:
		for next_room: Room in right_neighbour.next_rooms:
			if next_room.column < room.column:
				return true
	
	#no pueden cruzarse direccion izquierda si el vecino de la izquierda va a la derecha
...
func _setup_room_types() -> void:
	...
	#la mitad del camino es siempre treasure
	for room: Room in map_data[(FLOORS*0.5)-1]:
		if room.next_rooms.size() > 0:
			room.type = Room.Type.TREASURE 
	
	#pen ultimo piso antes del boss es un campfire
	for room: Room in map_data[FLOORS-2]:
		if room.next_rooms.size() > 0:
			room.type = Room.Type.CAMPFIRE
	
	#El resto de las rooms
